...
export default function MealCard({ recipe, mealType, day, onSwap, onSkip, onUnskip, compact = false }) {
  ...
  const RecipeIcon = ({ size = 'large' }) => {
    if (recipe.image && recipe.image.startsWith('http')) {
      return <img src={recipe.image} alt="" className="w-full h-full object-cover rounded-[inherit]" />;
    }
    if (recipe.image) {
      return <span className={size === 'small' ? 'text-2xl' : 'text-4xl'}>{recipe.image}</span>;
    }
